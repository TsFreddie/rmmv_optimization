...
(function () {
  ...
  var _KMS_Game_Interpreter_pluginCommand = Game_Interpreter.prototype.pluginCommand;
  ...
  Sprite_Minimap.prototype.updateVisibility = function () {
    this.visible = $gameSystem.isMinimapEnabled() && $gameMap.isMinimapEnabled();
  };
  ...
  Sprite_Minimap.prototype.refreshTilemap = function () {
    ...
    (Graphics.isWebGL() ? ShaderTilemap : Tilemap).prototype.update.call(this._tilemap);
  ...
  Sprite_Minimap.prototype.updatePosition = function () {
    ...
    if (this._lastPosition.x !== $gamePlayer.x || this._lastPosition.y !== $gamePlayer.y) {
      this.updateDrawRange();
      this.drawMap();
  ...
  Sprite_MinimapIcon.prototype.updateDisplaySize = function () {
    if (!KMS.imported['AreaEvent'] || !(this._object instanceof Game_Event)) {
      return;
    }
  ...
  //=============================================================================
  // HACK:
  //  1. Use fixed UInt8Array instead of Array for better memory access.
  //  2. Introduced dirty flags to only upload pixel changes when necessary.
  //  3. Position offset to fix shimmering when rendering map.
  //  4. Double buffer to temporally shift last render to reduce work.
  //=============================================================================
  Game_MinimapPassageCache.prototype.initialize = function (map, grid) {
    this._mapId = map.mapId();
    this._width = map.width();
    ...
    // HACK: memory optimization and dirty flags
    this._flags = new Uint8Array(this._width * this._height);
    this._dirty = new Uint8Array(this._width * this._height);

    this._blockChecked = new Array(this._blockCount.x * this._blockCount.y);
  };

  Game_MinimapPassageCache.prototype.setFlag = function (x, y, flag) {
    const key = x + y * this._width;
    if (this._flags[key] != flag) {
      this._dirty[key] = 1;
      this._flags[key] = flag;
    }
  };

  Game_MinimapPassageCache.prototype.clearDirty = function () {
    this._dirty.fill(0);
  };

  Game_MinimapPassageCache.prototype.isDirty = function (x, y) {
    return this._dirty[x + y * this._width];
  };
  ...
  Sprite_Minimap.prototype.initialize = function () {
    ...
    this._lastDrawRange = null;

    this._lastPosition = new Point($gamePlayer.x, $gamePlayer.y);

    // HACK: fix shimmering
    this._lastRealX = $gamePlayer._realX || 0;
    this._lastRealY = $gamePlayer._realY || 0;
    this._scrollDiff = new Point();
  ...
  Sprite_Minimap.prototype.createPassageSprite = function () {
    ...
    // HACK: double buffer
    this._passageSprite_swap = new Sprite_MinimapPassage(
      this._bitmapSize.width,
      this._bitmapSize.height
    );
  ...
  Sprite_Minimap.prototype.updateVisibility = function () {
    ...
    if (!wasVisible && this.visible) {
      // HACK: force an update to make sure offset value is processed
      this.updatePosition();
    }
  };

  Sprite_Minimap.prototype.refreshPassageTable = function () {
    const ogCache = this._passageCache;
    this._passageCache = this.getPassageTableCache();
    if (ogCache != this._passageCache) {
      // HACK: force clear cache and redraw
      this._passageSprite.bitmap.clear();
      this._lastDrawRange = null;
      this.drawMap();
    }
  };
  ...
  Sprite_Minimap.prototype.refreshMapImage = function () {
    // HACK: also clear cache when refreshing
    this._lastDrawRange = null;
    this._passageCache.clearDirty();
    this.drawMap();
  };

  Sprite_Minimap.prototype.updatePosition = function () {
    // HACK: reduce shimmering
    const realX = (this._lastRealX + $gamePlayer._realX) / 2;
    const realY = (this._lastRealY + $gamePlayer._realY) / 2;
    this._lastRealX = $gamePlayer._realX;
    this._lastRealY = $gamePlayer._realY;
    this._scrollDiff.x = (realX - $gamePlayer.x) * Params.gridSize;
    this._scrollDiff.y = (realY - $gamePlayer.y) * Params.gridSize;
    this.updateScroll();
  ...
  Sprite_Minimap.prototype.drawMap = function () {
    ...
    var bitmap = this._passageSprite.bitmap;
    // HACK: swap buffer each draw
    var swap = this._passageSprite_swap.bitmap;
    this._passageSprite.bitmap = swap;
    this._passageSprite_swap.bitmap = bitmap;
    swap.clear();
    this.drawMapForeground(swap, bitmap);
  };

  Sprite_Minimap.prototype.drawMapForeground = function (bitmap, lastBitmap) {
    if (this._lastDrawRange == null) {
      for (var y = this._drawRange.begin.y; y < this._drawRange.end.y; y++) {
        for (var x = this._drawRange.begin.x; x < this._drawRange.end.x; x++) {
          const gridState = this._getGridState(x, y);
          this.drawGridState(bitmap, gridState);
        }
      }
    } else {
      // HACK: find overlapping
      let sx = (this._drawRange.begin.x - this._lastDrawRange.x) * Params.gridSize;
      let sy = (this._drawRange.begin.y - this._lastDrawRange.y) * Params.gridSize;
      let dx = 0;
      let dy = 0;
      if (sx < 0) {
        dx = -sx;
      ...
      let width = bitmap.width - Math.abs(sx + dx);
      let height = bitmap.height - Math.abs(sy + dy);
      bitmap.blt(lastBitmap, sx, sy, width, height, dx, dy, width, height);

      const rangeWidth = this._drawRange.end.x - this._drawRange.begin.x;
      const rangeHeight = this._drawRange.end.y - this._drawRange.begin.y;
      const lastEndX = this._lastDrawRange.x + rangeWidth;
      const lastEndY = this._lastDrawRange.y + rangeHeight;
      for (var y = this._drawRange.begin.y; y < this._drawRange.end.y; y++) {
        for (var x = this._drawRange.begin.x; x < this._drawRange.end.x; x++) {
          if (
            x >= this._lastDrawRange.x &&
            x < lastEndX &&
            y >= this._lastDrawRange.y &&
            y < lastEndY
          ) {
            if (this._passageCache.isDirty(x, y)) {
              const gridState = this._getGridState(x, y);
              if (gridState.dw == 0 || gridState.dh == 0) {
                this._clearPassageAt(bitmap, x, y);
              } else {
                this.drawGridState(bitmap, gridState);
              }
            }
    ...
    this._passageCache.clearDirty();
    this._lastDrawRange = {
      x: this._drawRange.begin.x,
      y: this._drawRange.begin.y,
    };
  };

  const EMTPY_GRID_STATE = { dx: 0, dy: 0, dw: 0, dh: 0 };

  Sprite_Minimap.prototype._getGridState = function (x, y) {
    const passage = this._passageCache;
    if (!passage.isPassable(x, y)) {
      // HACK: reduce useless object creation
      return EMTPY_GRID_STATE;
    }

    let dx = x * Params.gridSize;
    let dy = y * Params.gridSize;
    let dw = Params.gridSize;
    let dh = Params.gridSize;
  ...
  Sprite_Minimap.prototype._clearPassageAt = function (bitmap, x, y) {
    var dx = (x - this._drawRange.begin.x) * Params.gridSize;
    var dy = (y - this._drawRange.begin.y) * Params.gridSize;
    var dw = Params.gridSize;
    var dh = Params.gridSize;

    bitmap.clearRect(dx, dy, dw, dh);
  };

  Sprite_Minimap.prototype.drawGridState = function (bitmap, gridState) {
    if (gridState.dw == 0 || gridState.dh == 0) return;

    const dx = gridState.dx - this._drawRange.begin.x * Params.gridSize;
    const dy = gridState.dy - this._drawRange.begin.y * Params.gridSize;

    bitmap.fillRect(dx, dy, gridState.dw, gridState.dh, Params.foregroundColor);
  };
})();
